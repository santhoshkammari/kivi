    ...
    class Wrapped(_BaseTool):
        ...
        def run(self, ctx: Context, request: ToolRequest) -> ToolResponse:
            try:
                ...
                return ToolResponse(f"[{self._instance.schema.name} error] {exc}", is_error=True)
...
def web_tools():
    """Return sync-wrapped web_search, web_fetch, run_markdown_agent for the CLI agent."""
    from ..tools.builtins import WebSearchTool, WebFetchTool, RunMarkdownAgentTool, register_builtin_tools
    from ..core.registry import Registry
    register_builtin_tools()
    return [_wrap(WebSearchTool)(), _wrap(WebFetchTool)(), _wrap(RunMarkdownAgentTool)()]
